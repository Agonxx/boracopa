        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {(() => {
            ...
            <div key={u.id} style={{ background: "var(--surface)", borderRadius: 14, border: "1px solid var(--line)", padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
              {/* avatar */}
              <div className="pii" style={{ width: 40, height: 40, borderRadius: "50%", flexShrink: 0, background: "var(--app-bg)", border: "1.5px solid var(--line-strong)", display: "grid", placeItems: "center", fontFamily: "Anton, sans-serif", fontSize: 15, color: "var(--ink-2)", letterSpacing: 0.3 }}>
                {(u.name ?? u.email).slice(0, 2).toUpperCase()}
              </div>

              {/* info */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <p className="pii" style={{ margin: "0 0 2px", fontWeight: 700, fontSize: 14, color: "var(--ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {u.name ?? <span style={{ color: "var(--ink-3)", fontStyle: "italic" }}>sem nome</span>}
                </p>
                <p className="pii" style={{ margin: 0, fontSize: 12.5, color: "var(--ink-2)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{u.email}</p>
                  ...
                  {bolaoMembersLoading ? (
                    ...
                    return (
                      <div key={m.user_id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 12, background: "var(--surface)", border: "1px solid var(--line)" }}>
                        ...
                        <span className="pii" style={{ flex: 1, fontSize: 13.5, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</span>
