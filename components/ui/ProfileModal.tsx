        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {editingName ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }} onClick={() => { setNameValue(user.name); setEditingName(true); }}>
              <div className="pii" style={{ fontSize: 17, fontWeight: 800, letterSpacing: -0.2, color: "var(--ink)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.name}</div>
              <Pencil size={13} strokeWidth={2} style={{ color: "var(--ink-3)", flexShrink: 0 }} />
            </div>
          )}
          <div className="pii" style={{ fontSize: 12, color: "var(--ink-2)", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email}</div>
        </div>
